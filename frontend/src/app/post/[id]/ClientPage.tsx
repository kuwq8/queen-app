...
import PostItem, { renderContentWithHashtags } from '../../../components/PostItem';
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'comments', filter: `post_id=eq.${postId}` }, async (payload) => {
          ...
          if (payload.new.user_id !== currentUserId) {
            ...
            setComments((prev: any[]) => {
              if (prev.some(c => c.id === (fullComment as any).id)) return prev;
      ...
      supabase.auth.getSession().then(({ data: { session } }) => {
        ...
        const insertComment = (url: string | null) => {
          supabase.from('comments').insert({
            ...
            media_url: url
          }).select('*, author:profiles!fk_comments_profiles(id, username, avatar_url)').single()
          ...
          {comments?.map(comment => (
            <div key={comment.id} className="p-4 border-b border-slate-800/50 flex gap-3 text-right hover:bg-[#111] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                {comment.content && <p className="text-slate-200 text-[15px] mt-1 break-words">{renderContentWithHashtags(comment.content)}</p>}
                {comment.media_url && (
