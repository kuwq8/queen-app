  ...
  const fetchLikedPosts = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('likes')
        .select('post:posts(*, author:profiles!posts_user_id_fkey(username, avatar_url))')
        .eq('user_id', profile?.id || session.user.id)
        .order('created_at', { ascending: false });
        
      if (!error && data) {
        setLikedPosts(data.map((l: any) => l.post).filter(Boolean));
      }
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="min-h-screen flex justify-center bg-black font-sans text-right">
      <div className="w-full max-w-[600px] flex flex-col relative border-x border-slate-800 min-h-screen bg-black">
        ...
        <div className="flex border-b border-slate-800 mb-4 mt-6">
          ...
          <button 
            onClick={() => { setActiveTab('likes'); fetchLikedPosts(); }}
              ...
              {bookmarkedPosts.length > 0 ? (
                bookmarkedPosts.map((post) => (
                  <PostItem 
                    ...
                    onPostEdited={(id, newContent) => { const update = (prev: any[]) => prev.map(p => p.id === id ? {...p, content: newContent} : p);   setPosts(update); }}
                  />
                ))
              ) : (
                <div className="text-center text-slate-500 py-10 font-bold">
