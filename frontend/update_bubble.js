...
export default function ChannelPostBubble({ post, currentUserId, onPostDeleted }: ChannelPostBubbleProps) {
  ...
  return (
    <div className="w-full flex justify-center mb-6 px-1 sm:px-2">
      <div className="w-[98%] sm:w-[90%] md:w-[80%] bg-[#202c33] rounded-2xl rounded-tr-none p-3 relative shadow-md">
        ...
        {currentUserId === post.user_id && (
          <button 
            ...
            className="absolute top-3 left-3 text-slate-500 hover:text-red-500 transition-colors bg-[#182229]/50 p-1.5 rounded-full"
            title="حذف البث"
          >
            <Trash2 size={16} />
          </button>
        ...
        {/* Bottom row: Time and WhatsApp-style Reactions Pill */}
        <div className="absolute bottom-2 left-3 text-[11px] text-slate-400 font-medium">
          {timeString}
        </div>

        {/* Reactions floating exactly at bottom right overlapping the border like WhatsApp */}
        <div className="absolute -bottom-3 right-4 flex gap-1.5 items-center z-10" ref={pickerRef}>
          ...
          <div className="relative">
            ...
            {showEmojiPicker && (
              <div className="absolute bottom-full right-0 mb-2 bg-[#2a3942] border border-slate-600 rounded-full py-2 px-3 flex gap-2 shadow-2xl z-50">
                {availableEmojis.map(emoji => {
                  const isSelected = groupedReactions[emoji]?.me;
                  return (
                    <button 
                      key={emoji}
