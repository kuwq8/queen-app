...
const newRender = `  // Filter conversations
...
// Split the file using a regular expression that handles both \n and \r\n
const lines = content.split(/\r?\n/);

const renderStartIdx = lines.findIndex(l => l.includes('return (') && l.includes('bg-black') === false && lines[lines.indexOf(l)+1].includes('h-full flex flex-col relative bg-black'));
const modalStartIdx = lines.findIndex(l => l.includes('{/* New Chat Modal */}'));

if (renderStartIdx !== -1 && modalStartIdx !== -1) {
  const beforeRender = lines.slice(0, renderStartIdx).join('\n');
  const afterModal = lines.slice(modalStartIdx + 1).join('\n');
  const finalContent = beforeRender + '\n' + newRender + '\n' + afterModal;
  fs.writeFileSync('src/app/messages/MessagesSidebar.tsx', finalContent);
  console.log('done');
} else {
  console.log('Could not find indices', renderStartIdx, modalStartIdx);
}
